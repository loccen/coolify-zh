...
it('wires init command phase one strings through console translations', function () {
    ...
    expect($command)
        ...
        ->toContain("trans('console.init.info.marked_stuck_deployments_as_failed', ['count' => \$updatedCount], locale: \$locale)")
        ->toContain("trans('console.init.info.marked_stuck_scheduled_task_executions_as_failed', ['count' => \$updatedTaskCount], locale: \$locale)")
        ->toContain("trans('console.init.info.marked_stuck_database_backup_executions_as_failed', ['count' => \$updatedBackupCount], locale: \$locale)")
    ...
    expect($enTranslations['init']['description'])->toBe('Cleanup instance related stuffs')
        ...
        ->and($enTranslations['init']['info']['marked_stuck_deployments_as_failed'])->toBe('Marked :count stuck deployments as failed.')
        ->and($enTranslations['init']['info']['marked_stuck_scheduled_task_executions_as_failed'])->toBe('Marked :count stuck scheduled task executions as failed.')
        ->and($enTranslations['init']['info']['marked_stuck_database_backup_executions_as_failed'])->toBe('Marked :count stuck database backup executions as failed.')
    ...
    expect($zhTranslations['init']['description'])->toBe('清理实例相关内容')
        ...
        ->and($zhTranslations['init']['info']['marked_stuck_deployments_as_failed'])->toBe('已将 :count 个卡住的部署标记为失败。')
        ->and($zhTranslations['init']['info']['marked_stuck_scheduled_task_executions_as_failed'])->toBe('已将 :count 条卡住的计划任务执行记录标记为失败。')
        ->and($zhTranslations['init']['info']['marked_stuck_database_backup_executions_as_failed'])->toBe('已将 :count 条卡住的数据库备份执行记录标记为失败。')
...
it('resolves init command phase one translations in en and zh_CN', function () {
    ...
    expect(trans('console.init.description'))->toBe('Cleanup instance related stuffs')
        ...
        ->and(trans('console.init.info.marked_stuck_deployments_as_failed', ['count' => 3]))->toBe('Marked 3 stuck deployments as failed.')
        ->and(trans('console.init.info.marked_stuck_scheduled_task_executions_as_failed', ['count' => 2]))->toBe('Marked 2 stuck scheduled task executions as failed.')
        ->and(trans('console.init.info.marked_stuck_database_backup_executions_as_failed', ['count' => 1]))->toBe('Marked 1 stuck database backup executions as failed.')
    ...
    expect(trans('console.init.description'))->toBe('清理实例相关内容')
        ...
        ->and(trans('console.init.info.marked_stuck_deployments_as_failed', ['count' => 3]))->toBe('已将 3 个卡住的部署标记为失败。')
        ->and(trans('console.init.info.marked_stuck_scheduled_task_executions_as_failed', ['count' => 2]))->toBe('已将 2 条卡住的计划任务执行记录标记为失败。')
        ->and(trans('console.init.info.marked_stuck_database_backup_executions_as_failed', ['count' => 1]))->toBe('已将 1 条卡住的数据库备份执行记录标记为失败。')
