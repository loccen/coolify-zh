...
it('wires generate services console strings through console translations', function () {
    ...
    expect($enTranslations['generate_services']['info']['ignoring'])->toBe('Ignoring :file')
        ->and($enTranslations['generate_services']['info']['processing'])->toBe('Processing :file');

    expect($zhTranslations['generate_services']['info']['ignoring'])->toBe('忽略 :file')
...
it('resolves generate services translations in en and zh_CN', function () {
    ...
    expect(trans('console.generate_services.info.ignoring', ['file' => 'demo.yaml']))->toBe('Ignoring demo.yaml')
    ...
    expect(trans('console.generate_services.info.ignoring', ['file' => 'demo.yaml']))->toBe('忽略 demo.yaml')
