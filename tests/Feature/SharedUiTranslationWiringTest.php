...
it('wires translation calls in shared ui follow-up files', function () {
    ...
    expect($modalConfirmation)
        ...
        ->toContain("__('The content shown may be outdated. Click \\\"Load from server\\\" to fetch the latest version.')")
        ->and($serviceStorage)
        ->toContain("{{ __('Volumes') }}")
        ->toContain("__('No file mounts configured.')")
        ->toContain("__('Volume Mount')")
        ->toContain("__('File Mount')")
        ->toContain("__('Directory Mount')")
        ->and($serviceIndex)
...
it('resolves representative shared ui follow-up translations in zh_CN', function () {
    ...
    expect(__('Deleted database'))->toBe('已删除的数据库')
        ...
        ->and(__('Volume Mount'))->toBe('卷挂载')
        ->and(__('File Mount'))->toBe('文件挂载')
        ->and(__('Directory Mount'))->toBe('目录挂载')
        ->and(__('The content shown may be outdated. Click \"Load from server\" to fetch the latest version.'))->toBe('当前显示的内容可能已经过期。点击“从服务器加载”以获取最新版本。')
        ->and(__('Volume added successfully'))->toBe('卷已添加。')
        ->and(__('File mount added successfully'))->toBe('文件挂载已添加。')
        ->and(__('Directory mount added successfully'))->toBe('目录挂载已添加。')
        ->and(__('File storage loaded from server.'))->toBe('已从服务器加载文件存储内容。')
        ->and(__('File updated.'))->toBe('文件已更新。')
