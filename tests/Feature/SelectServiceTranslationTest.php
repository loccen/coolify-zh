...
it('translates one-click service slogans and database descriptions on the select page', function () {
    ...
    expect($services->get('Activepieces')['slogan'])->toBe('开源无代码业务自动化平台。')
        ...
        ->and($services->get('Heimdall')['slogan'])->toBe('Heimdall 是一个用于管理和整理服务器应用的仪表板。')
        ->and($services->get('Miniflux')['slogan'])->toBe('Miniflux 是一个极简且有鲜明取舍的订阅阅读器。')
        ->and($services->get('Netbird Client')['slogan'])->toBe('将你的设备接入一个基于 WireGuard® 的安全覆盖网络，并支持 SSO、MFA 和细粒度访问控制。')
        ->and($services->get('Nextcloud')['slogan'])->toBe('NextCloud 是一个自托管、开源的平台，提供文件存储、协作和通信工具，用于顺畅地管理数据。')
