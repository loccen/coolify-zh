...
it('excludes current version of Coolify infrastructure images from any registry', function () {
    ...
    // Pattern: (^|/)loccen/coolify-helper:VERSION$|(^|/)loccen/coolify-realtime:VERSION$
    ...
    $infraPattern = "(^|\\/)loccen\\/coolify-helper:{$escapedHelperVersion}$|(^|\\/)loccen\\/coolify-realtime:{$escapedRealtimeVersion}$";
    $pattern = "/{$infraPattern}/";

    // Current versioned infrastructure images from ANY registry should be PROTECTED
    $protectedImages = [
        // ghcr.io registry
        "ghcr.io/loccen/coolify-helper:{$helperVersion}",
        "ghcr.io/loccen/coolify-realtime:{$realtimeVersion}",
        // docker.io registry (explicit)
        "docker.io/loccen/coolify-helper:{$helperVersion}",
        "docker.io/loccen/coolify-realtime:{$realtimeVersion}",
        // No registry prefix (Docker Hub implicit)
        "loccen/coolify-helper:{$helperVersion}",
        "loccen/coolify-realtime:{$realtimeVersion}",
    ];
    ...
    $oldVersionImages = [
        'ghcr.io/loccen/coolify-helper:1.0.11',
        'docker.io/loccen/coolify-helper:1.0.10',
        'loccen/coolify-helper:1.0.9',
        'ghcr.io/loccen/coolify-realtime:1.0.9',
        'ghcr.io/loccen/coolify-helper:latest',
        'loccen/coolify-realtime:latest',
    ];
...
it('protects current infrastructure images from any registry even when no applications exist', function () {
    ...
    expect(preg_match($pattern, "ghcr.io/loccen/coolify-helper:{$helperVersion}"))->toBe(1);
    expect(preg_match($pattern, "docker.io/loccen/coolify-helper:{$helperVersion}"))->toBe(1);
    expect(preg_match($pattern, "loccen/coolify-helper:{$helperVersion}"))->toBe(1);
    expect(preg_match($pattern, "ghcr.io/loccen/coolify-realtime:{$realtimeVersion}"))->toBe(1);

    // Old versions should NOT be protected
    expect(preg_match($pattern, 'ghcr.io/loccen/coolify-helper:1.0.11'))->toBe(0);
    expect(preg_match($pattern, 'docker.io/loccen/coolify-helper:1.0.11'))->toBe(0);
