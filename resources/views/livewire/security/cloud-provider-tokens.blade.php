    ...
    <div class="grid gap-2 lg:grid-cols-1">
        @forelse ($tokens as $savedToken)
            <div wire:key="token-{{ $savedToken->id }}"
                ...
                <div class="text-sm">{{ __('Created: :time', ['time' => $savedToken->created_at->diffForHumans()]) }}</div>
                ...
                <div class="flex gap-2 pt-2">
                    ...
                    @can('delete', $savedToken)
                        <x-modal-confirmation :title="__('Confirm Token Deletion?')" isErrorButton :buttonTitle="__('Delete')"
                            submitAction="deleteToken({{ $savedToken->id }})" :actions="[
                                __('This cloud provider token will be permanently deleted.'),
                                __('Any servers using this token will need to be reconfigured.'),
                            ]"
                            confirmationText="{{ $savedToken->name }}"
                            :confirmationLabel="__('Please confirm the deletion by entering the token name below')"
                            :shortConfirmationLabel="__('Token Name')" :confirmWithPassword="false" :step2ButtonText="__('Delete Token')" />
                    @endcan
                </div>
            </div>
        @empty
