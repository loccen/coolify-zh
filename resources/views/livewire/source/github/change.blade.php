                ...
                <div class="flex gap-2">
                    ...
                    @can('delete', $github_app)
                        @if ($applications->count() > 0)
                            <x-modal-confirmation :title="__('Confirm GitHub App Deletion?')" isErrorButton :buttonTitle="__('Delete')"
                                submitAction="delete" :actions="[__('The selected GitHub App will be permanently deleted.')]" confirmationText="{{ data_get($github_app, 'name') }}"
                                :confirmationLabel="__('Please confirm the execution of the actions by entering the GitHub App Name below')"
                                :shortConfirmationLabel="__('GitHub App Name')" :confirmWithPassword="false"
                                :step2ButtonText="__('Permanently Delete')" />
                        @else
                            <x-modal-confirmation :title="__('Confirm GitHub App Deletion?')" isErrorButton :buttonTitle="__('Delete')"
                                submitAction="delete" :actions="[__('The selected GitHub App will be permanently deleted.')]"
                                :confirmationLabel="__('Please confirm the execution of the actions by entering the GitHub App Name below')"
                                :shortConfirmationLabel="__('GitHub App Name')"
                ...
                <div class="flex flex-col gap-2">
                    <div class="flex flex-col sm:flex-row gap-2">
                        <div class="flex flex-col sm:flex-row items-start sm:items-end gap-2 w-full">
                            <x-forms.input canGate="update" :canResource="$github_app" id="name" :label="__('App Name')" />
                    ...
                    <x-forms.input canGate="update" :canResource="$github_app" id="organization" :label="__('Organization')"
                        :placeholder="__('If empty, personal user will be used')" />
                    @if (!isCloud())
                        <div class="w-48">
                            <x-forms.checkbox canGate="update" :canResource="$github_app" :label="__('System Wide?')"
                                :helper="__('If checked, this GitHub App will be available for everyone in this Coolify instance.')"
                                instantSave id="isSystemWide" />
                        </div>
                        @if ($isSystemWide)
                            <x-callout type="warning" :title="__('Not Recommended')">
                                {{ __('System-wide GitHub Apps are shared across all teams on this Coolify instance. This means any team can use this GitHub App to deploy applications from your repositories. For better security and isolation, it is recommended to create team-specific GitHub Apps instead.') }}
                            </x-callout>
                        @endif
                    @endif
                    <div class="flex flex-col sm:flex-row gap-2">
                        <x-forms.input canGate="update" :canResource="$github_app" id="htmlUrl" :label="__('HTML Url')" />
                        <x-forms.input canGate="update" :canResource="$github_app" id="apiUrl" :label="__('API Url')" />
                    </div>
                    <div class="flex flex-col sm:flex-row gap-2">
                        <x-forms.input canGate="update" :canResource="$github_app" id="customUser" :label="__('User')"
                            required />
                        <x-forms.input canGate="update" :canResource="$github_app" type="number" id="customPort"
                            :label="__('Port')" required />
                    </div>
                    <div class="flex flex-col sm:flex-row gap-2">
                        <x-forms.input canGate="update" :canResource="$github_app" type="number" id="appId"
                            :label="__('App Id')" required />
                        <x-forms.input canGate="update" :canResource="$github_app" type="number"
                            id="installationId" :label="__('Installation Id')" required />
                    </div>
                    <div class="flex flex-col sm:flex-row gap-2">
                        <x-forms.input canGate="update" :canResource="$github_app" id="clientId" :label="__('Client Id')"
                            type="password" required />
                        <x-forms.input canGate="update" :canResource="$github_app" id="clientSecret"
                            :label="__('Client Secret')" type="password" required />
                        <x-forms.input canGate="update" :canResource="$github_app" id="webhookSecret"
                            :label="__('Webhook Secret')" type="password" required />
                    </div>
                    <div class="flex gap-2">
                        <x-forms.select canGate="update" :canResource="$github_app" id="privateKeyId"
                            :label="__('Private Key')" required>
        ...
        <div class="flex flex-col gap-2">
            ...
            <div class="flex flex-col">
                <div class="pb-10">
                    @can('create', $github_app)
                        @if (!isCloud() || isDev())
                            <div class="flex flex-col sm:flex-row items-start sm:items-end gap-2">
                                <x-forms.select wire:model.live='webhook_endpoint' :label="__('Webhook Endpoint')"
                                    :helper="__('All Git webhooks will be sent to this endpoint. <br><br>If you would like to use a domain instead of an IP address, set your Coolify instance FQDN in the Settings menu.')">
                                    @if ($fqdn)
                        ...
                        <div class="flex w-full flex-col gap-2 pt-4 sm:w-96">
                            <x-forms.checkbox disabled id="default_permissions" :label="__('Mandatory')"
                                :helper="__('Contents: read<br>Metadata: read<br>Email: read')" />
                            <x-forms.checkbox id="preview_deployment_permissions" :label="__('Preview Deployments')"
                                :helper="__('Necessary for updating pull requests with useful comments (deployment status, links, etc.)<br><br>Pull Request: read & write')" />
                    ...
                    @else
                        <x-callout type="danger" :title="__('Insufficient Permissions')">
                            {{ __('You do not have permission to create new GitHub Apps. Please contact your team administrator.') }}
                        </x-callout>
                    @endcan
                </div>
            </div>
            <script>
                function createGithubApp(webhook_endpoint, preview_deployment_permissions, administration) {
                    ...
                    if (!webhook_endpoint) {
                        alert(@js(__('Please select a webhook endpoint.')));
                        return;
                    }
