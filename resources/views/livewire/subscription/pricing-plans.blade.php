<div x-data="{ selected: 'monthly' }" class="w-full">
    ...
    <div class="max-w-xl mx-auto">
        {{-- Plan Header + Pricing --}}
        <h3 id="tier-dynamic" class="text-2xl font-bold dark:text-white">{{ __('Pay-as-you-go') }}</h3>
        <p class="mt-1 text-sm dark:text-neutral-400">{{ __('Dynamic pricing based on the number of servers you connect.') }}</p>

        <div class="mt-4 flex items-baseline gap-x-1">
            <span x-show="selected === 'monthly'" x-cloak>
                <span class="text-4xl font-bold tracking-tight dark:text-white">$5</span>
                <span class="text-sm dark:text-neutral-400">{{ __('/ mo base') }}</span>
            </span>
        ...
        <p class="mt-1 text-sm dark:text-neutral-400">
            <span x-show="selected === 'monthly'" x-cloak>
                {!! __('+ <span class="font-semibold dark:text-white">$3</span> per additional server, billed monthly (+VAT)') !!}
            </span>
            <span x-show="selected === 'yearly'" x-cloak>
                {!! __('+ <span class="font-semibold dark:text-white">$2.7</span> per additional server, billed annually (+VAT)') !!}
            </span>
        </p>
        ...
        <div class="mt-8 pt-6 border-t dark:border-coolgray-400 border-neutral-200">
            <ul role="list" class="space-y-2.5 text-sm">
                <li class="flex items-center gap-2.5">
                    ...
                    <span class="dark:text-neutral-300">{{ __('Connect unlimited servers') }}</span>
                </li>
                <li class="flex items-center gap-2.5">
                    <svg class="w-4 h-4 shrink-0 dark:text-warning" viewBox="0 0 20 20" fill="currentColor"
                        aria-hidden="true">
                    ...
                    <span class="dark:text-neutral-300">{{ __('Deploy unlimited applications per server') }}</span>
                </li>
                <li class="flex items-center gap-2.5">
                    <svg class="w-4 h-4 shrink-0 dark:text-warning" viewBox="0 0 20 20" fill="currentColor"
                        aria-hidden="true">
                    ...
                    <span class="dark:text-neutral-300">{{ __('Free email notifications') }}</span>
                </li>
                <li class="flex items-center gap-2.5">
                    <svg class="w-4 h-4 shrink-0 dark:text-warning" viewBox="0 0 20 20" fill="currentColor"
                        aria-hidden="true">
                    ...
                    <span class="dark:text-neutral-300">{{ __('Support by email') }}</span>
                </li>
        ...
        <div class="mt-6 pt-6 border-t dark:border-coolgray-400 border-neutral-200 text-sm dark:text-neutral-400">
            <p>{!! __('You need to bring your own servers from any cloud provider (<a class="underline" href="https://coolify.io/hetzner" target="_blank">Hetzner</a>, DigitalOcean, AWS, etc.) or connect any device running a <a class="underline" href="https://coolify.io/docs/installation#supported-operating-systems" target="_blank">supported OS</a>.') !!}</p>
            <p class="mt-3">{!! __('Need official support for your self-hosted instance? <a class="underline dark:text-white" href="https://coolify.io/docs/contact" target="_blank">Contact Us</a>') !!}</p>
        </div>
    </div>
</div>
