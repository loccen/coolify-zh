    ...
    <div class="flex flex-col h-full gap-8 sm:flex-row">
        ...
        <div class="w-full">
            <form wire:submit.prevent='submit' class="flex flex-col">
                <div class="flex gap-2">
                    ...
                    @if ($server->hetzner_server_id)
                        <div class="flex items-center">
                            ...
                            <button wire:loading.remove wire:target="checkHetznerServerStatus" title="{{ __('Refresh Status') }}"
                            ...
                            <button wire:loading wire:target="checkHetznerServerStatus" title="{{ __('Refreshing Status') }}"
                                class="mx-1 dark:hover:fill-white fill-black dark:fill-warning">
                    ...
                    @endif
                    @if ($server->id === 0)
                        <x-modal-confirmation :title="__('Confirm Server Settings Change?')" :buttonTitle="__('Save')"
                            submitAction="submit" :actions="[
                                __('If you misconfigure the server, you could lose a lot of functionalities of Coolify.'),
                            ]" :confirmWithText="false" :confirmWithPassword="false"
                            :step2ButtonText="__('Save')" canGate="update" :canResource="$server" />
                    @else
                ...
                @if ($server->isFunctional())
                    {{ __('Server is reachable and validated.') }}
                @else
                    {{ __("You can't use this server until it is validated.") }}
                @endif
                @if ($isValidating)
                ...
                @if ($server->isForceDisabled() && isCloud())
                    <x-callout type="danger" :title="__('Server Disabled')" class="mt-4">
                ...
                <div class="flex flex-col gap-2 pt-4">
                    <div class="flex flex-col gap-2 w-full lg:flex-row">
                        <x-forms.input canGate="update" :canResource="$server" id="name" :label="__('Name')" required
                            :disabled="$isValidating" />
                        <x-forms.input canGate="update" :canResource="$server" id="description" :label="__('Description')"
                            :disabled="$isValidating" />
                    ...
                    <div class="flex flex-col gap-2 w-full lg:flex-row">
                        <x-forms.input canGate="update" :canResource="$server" type="password" id="ip"
                            :label="__('IP Address/Domain')"
                            :helper="__('An IP Address (127.0.0.1) or domain (example.com). Make sure there is no protocol like http(s):// so you provide a FQDN not a URL.')"
                            required :disabled="$isValidating" />
                        <div class="flex gap-2">
                            <x-forms.input canGate="update" :canResource="$server" id="user" :label="__('User')" required
                                :disabled="$isValidating" />
                            <x-forms.input canGate="update" :canResource="$server" type="number" id="port"
                                :label="__('Port')" required :disabled="$isValidating" />
                        </div>
                    </div>
                    <div class="w-full lg:w-64">
                        <x-forms.input canGate="update" :canResource="$server" type="number"
                            id="connectionTimeout" :label="__('SSH Connection Timeout (s)')"
                            :helper="__('Seconds to wait for SSH connection before failing. Default: 10.')"
                    ...
                    <div class="w-full">
                        <div class="flex items-center mb-1">
                            ...
                            <x-helper class="ml-2"
                                :helper='__("Server\'s timezone. This is used for backups, cron jobs, etc.")' />
                        </div>
                        @can('update', $server)
                            @if ($isValidating)
                                <div class="relative">
                                    <div class="inline-flex relative items-center w-64">
                                        <input readonly disabled autocomplete="off"
                                            ...
                                            value="{{ $serverTimezone ?: __('No timezone set') }}"
                                            placeholder="{{ __('Server Timezone') }}">
                            ...
                            @else
                                <div x-data="{
                                    ...
                                    placeholder: @js($serverTimezone ? __('Search timezone...') : __('Select Server Timezone')),
                                    init() {
                        ...
                        @else
                            <div class="relative">
                                <div class="inline-flex relative items-center w-64">
                                    <input readonly disabled autocomplete="off"
                                        class="w-full input opacity-50 cursor-not-allowed"
                                        value="{{ $serverTimezone ?: __('No timezone set') }}" placeholder="{{ __('Server Timezone') }}">
                                    <svg class="absolute right-0 mr-2 w-4 h-4 opacity-50"
                                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke-width="1.5" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <div class="w-full sm:w-96">
                                @if ($isBuildServerLocked)
                                    <x-forms.checkbox disabled instantSave id="isBuildServer"
                                        :helper='__("You can\'t use this server as a build server because it has defined resources.")'
                                        :label="__('Use it as a build server?')" />
                                @else
                                    <x-forms.checkbox canGate="update" :canResource="$server" instantSave
                                        id="isBuildServer" :label="__('Use it as a build server?')" :disabled="$isValidating" />
                                @endif
                            </div>
                ...
                <div class="pt-6">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        @if ($server->server_metadata)
                            <button wire:click="refreshServerMetadata" wire:loading.attr="disabled"
                                wire:target="refreshServerMetadata" title="{{ __('Refresh server details') }}"
                    ...
                    @if ($server->server_metadata)
                        ...
                        <div class="grid grid-cols-2 gap-x-6 gap-y-2 text-sm lg:grid-cols-3">
                            <div><span class="font-medium dark:text-neutral-400">{{ __('OS') }}:</span>
                                {{ $meta['os'] ?? 'N/A' }}</div>
                            <div><span class="font-medium dark:text-neutral-400">{{ __('Arch') }}:</span>
                                {{ $meta['arch'] ?? 'N/A' }}</div>
                            <div><span class="font-medium dark:text-neutral-400">{{ __('Kernel') }}:</span>
                                {{ $meta['kernel'] ?? 'N/A' }}</div>
                            <div><span class="font-medium dark:text-neutral-400">{{ __('CPU Cores') }}:</span>
                                {{ $meta['cpus'] ?? 'N/A' }}</div>
                            <div><span class="font-medium dark:text-neutral-400">{{ __('RAM') }}:</span>
                                {{ isset($meta['memory_bytes']) ? round($meta['memory_bytes'] / 1073741824, 1) . ' GB' : 'N/A' }}
                            </div>
                            <div><span class="font-medium dark:text-neutral-400">{{ __('Up Since') }}:</span>
                                {{ $meta['uptime_since'] ?? 'N/A' }}</div>
                        </div>
                        @if (isset($meta['collected_at']))
                            <p class="mt-2 text-xs dark:text-neutral-500">{{ __('Last updated:') }}
                                {{ \Carbon\Carbon::parse($meta['collected_at'])->diffForHumans() }}</p>
                        @endif
                    @else
                        <x-forms.button wire:click="refreshServerMetadata" canGate="update"
                            :canResource="$server">
                            <span wire:loading.remove wire:target="refreshServerMetadata">{{ __('Fetch Server Details') }}</span>
                            <span wire:loading wire:target="refreshServerMetadata">{{ __('Fetching...') }}</span>
                        </x-forms.button>
                    @endif
                </div>
            @endif
            @if (!$server->hetzner_server_id && $availableHetznerTokens->isNotEmpty())
                <div class="pt-6">
                    <h3>{{ __('Link to Hetzner Cloud') }}</h3>
                    <p class="pb-4 text-sm dark:text-neutral-400">
                        {{ __('Link this server to a Hetzner Cloud instance to enable power controls and status monitoring.') }}
                    </p>

                    <div class="flex flex-wrap gap-4 items-end">
                        <div class="w-72">
                            <x-forms.select wire:model="selectedHetznerTokenId" :label="__('Hetzner Token')"
                        ...
                        <div class="w-48">
                            <x-forms.input wire:model="manualHetznerServerId"
                                ...
                                :helper="__('Enter the Hetzner Server ID from your Hetzner Cloud console')"
                                canGate="update" :canResource="$server" />
                        </div>
                        <x-forms.button wire:click="searchHetznerServerById"
                            wire:loading.attr="disabled"
                            canGate="update" :canResource="$server">
                            <span wire:loading.remove wire:target="searchHetznerServerById">{{ __('Search by ID') }}</span>
                            <span wire:loading wire:target="searchHetznerServerById">{{ __('Searching...') }}</span>
                        </x-forms.button>
                        <div class="self-end pb-2 text-sm dark:text-neutral-500">{{ __('OR') }}</div>
                        <x-forms.button wire:click="searchHetznerServer"
                            wire:loading.attr="disabled"
                            canGate="update" :canResource="$server">
                            <span wire:loading.remove wire:target="searchHetznerServer">{{ __('Search by IP') }}</span>
                            <span wire:loading wire:target="searchHetznerServer">{{ __('Searching...') }}</span>
                        </x-forms.button>
                    </div>
                        ...
                        <div class="mt-4 p-4 border border-yellow-500 rounded-md bg-yellow-50 dark:bg-yellow-900/20">
                            <p class="text-yellow-600 dark:text-yellow-400">
                                @if ($manualHetznerServerId)
                                    {{ __('No Hetzner server found with ID: :id', ['id' => $manualHetznerServerId]) }}
                                @else
                                    {{ __('No Hetzner server found matching IP: :ip', ['ip' => $server->ip]) }}
                                @endif
                            </p>
                            <p class="text-sm dark:text-neutral-400 mt-1">
                                {{ __('Try a different token, enter the Server ID manually, or verify the details are correct.') }}
                            </p>
                        </div>
                    @endif
                    ...
                    @if ($matchedHetznerServer)
                        <div class="mt-4 p-4 border border-green-500 rounded-md bg-green-50 dark:bg-green-900/20">
                            <h4 class="font-semibold text-green-700 dark:text-green-400 mb-2">{{ __('Match Found!') }}</h4>
                            <div class="grid grid-cols-2 gap-2 text-sm mb-4">
                                <div><span class="font-medium">{{ __('Name') }}:</span> {{ $matchedHetznerServer['name'] }}</div>
                                <div><span class="font-medium">{{ __('ID') }}:</span> {{ $matchedHetznerServer['id'] }}</div>
                                <div><span class="font-medium">{{ __('Status') }}:</span> {{ ucfirst($matchedHetznerServer['status']) }}</div>
                                <div><span class="font-medium">{{ __('Type') }}:</span> {{ data_get($matchedHetznerServer, 'server_type.name', __('Unknown')) }}</div>
                            </div>
