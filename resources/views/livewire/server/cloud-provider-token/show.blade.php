<div>
    <x-slot:title>
        {{ data_get_str($server, 'name')->limit(10) }} > {{ __('Hetzner Token') }} | Coolify
    </x-slot>
        ...
        <div class="w-full">
            @if ($server->hetzner_server_id)
                <div class="flex items-end gap-2">
                    <h2>{{ __('Hetzner Token') }}</h2>
                    @can('create', App\Models\CloudProviderToken::class)
                        <x-modal-input :buttonTitle="__('+ Add')" :title="__('Add Hetzner Token')">
                ...
                <div class="pb-4">{{ __("Change your server's Hetzner token.") }}</div>
                <div class="grid xl:grid-cols-2 grid-cols-1 gap-2">
                    ...
                    @empty
                        <div>{{ __('No Hetzner tokens found.') }}</div>
                    @endforelse
                </div>
            @else
                <div class="flex items-end gap-2">
                ...
                <div class="pb-4">{{ __('This server was not created through Hetzner Cloud integration.') }}</div>
                <div class="p-4 border rounded-md dark:border-coolgray-300 dark:bg-coolgray-100">
                    <p class="dark:text-neutral-400">
                        {{ __('Only servers created through Hetzner Cloud can have their tokens managed here.') }}
                    </p>
                </div>
            @endif
        </div>
