                    ...
                    <div class="w-full sm:w-96 flex flex-col gap-1 pt-4">
                        @if (str($serviceApplication->image)->contains('pocketbase'))
                            <x-forms.checkbox canGate="update" :canResource="$serviceApplication" instantSave="instantSaveApplicationSettings" id="isGzipEnabled"
                                :label="__('Enable Gzip Compression')"
                                :helper="__('Pocketbase does not need gzip compression, otherwise SSE will not work.')" disabled />
                        @else
                            <x-forms.checkbox canGate="update" :canResource="$serviceApplication" instantSave="instantSaveApplicationSettings" id="isGzipEnabled"
                                :label="__('Enable Gzip Compression')"
                        ...
                        <x-forms.checkbox canGate="update" :canResource="$serviceApplication" instantSave="instantSaveApplicationSettings" id="isStripprefixEnabled"
                            :label="__('Strip Prefixes')"
                            :helper="__('Strip Prefix is used to remove prefixes from paths. Like /api/ to /api.')" />
                        <x-forms.checkbox canGate="update" :canResource="$serviceApplication" instantSave="instantSaveApplicationSettings" :label="__('Exclude from service status')"
                            :helper="__('If you do not need to monitor this resource, enable. Useful if this service is optional.')"
                            id="excludeFromStatus"></x-forms.checkbox>
                        <x-forms.checkbox canGate="update" :canResource="$serviceApplication"
                            :helper="__('Drain logs to your configured log drain endpoint in your Server settings.')"
                            instantSave="instantSaveApplicationAdvanced" id="isLogDrainEnabled" :label="__('Drain Logs')" />
                    </div>
                @else
                    <form wire:submit='submitApplication'>
                        ...
                        <div class="flex flex-col gap-2">
                            @if ($requiredPort && !$serviceApplication->serviceType()?->contains(str($serviceApplication->image)->before(':')))
                                <x-callout type="info" :title="__('Required Port: :port', ['port' => $requiredPort])" class="mb-2">
                                    {!! __('This service requires port <strong>:port</strong> to function correctly. All domains must include this port number (or any other port if you know what you\'re doing).', ['port' => $requiredPort]) !!}
                                    <br><br>
                                ...
                                <div x-show="modalOpen"
                                    ...
                                    <div x-show="modalOpen" x-trap.inert.noscroll="modalOpen" x-transition:enter="ease-out duration-100"
                                        ...
                                        <div class="flex justify-between items-center pb-3">
                                            <h2 class="pr-8 font-bold">{{ __('Remove Required Port?') }}</h2>
                                        ...
                                        <div class="relative w-auto">
                                            <x-callout type="warning" :title="__('Port Requirement Warning')" class="mb-4">
                                                {!! __('This service requires port <strong>:port</strong> to function correctly.', ['port' => $requiredPort]) !!}
                                            ...
                                            <div class="flex flex-wrap gap-2 justify-between mt-4">
                                                ...
                                                <x-forms.button wire:click="confirmRemovePort" @click="modalOpen = false" class="w-auto"
                                                    isError>
                                                    {{ __('I understand, remove port anyway') }}
                                                </x-forms.button>
                                            </div>
                                        </div>
                                    </div>
                ...
                @elseif ($currentRoute === 'project.service.index.advanced')
                    ...
                    <div class="w-full sm:w-96 flex flex-col gap-1 pt-4">
                        <x-forms.checkbox canGate="update" :canResource="$serviceDatabase" instantSave="instantSaveExclude"
                            :label="__('Exclude from service status')"
                            :helper="__('If you do not need to monitor this resource, enable. Useful if this service is optional.')"
                            id="excludeFromStatus"></x-forms.checkbox>
                        <x-forms.checkbox canGate="update" :canResource="$serviceDatabase"
                            :helper="__('Drain logs to your configured log drain endpoint in your Server settings.')"
                            instantSave="instantSaveLogDrain" id="isLogDrainEnabled" :label="__('Drain Logs')" />
                    </div>
                @else
                            ...
                            <div class="flex flex-col gap-2">
                                ...
                                <div class="flex flex-col gap-2 w-64">
                                    <x-forms.checkbox canGate="update" :canResource="$serviceDatabase" instantSave id="isPublic"
                                        :label="__('Make it publicly available')" />
                                </div>
                                <x-forms.input type="number" canGate="update" :canResource="$serviceDatabase" placeholder="5432"
                                    disabled="{{ $serviceDatabase->is_public }}" id="publicPort" :label="__('Public Port')" />
                                @if ($db_url_public)
