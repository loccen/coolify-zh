    ...
    <form wire:submit.prevent='submit' class="flex flex-col w-full gap-2">
        @if($requiredPort)
            <x-callout type="info" :title="__('Required Port: :port', ['port' => $requiredPort])" class="mb-2">
                {!! __('This service requires port <strong>:port</strong> to function correctly. All domains must include this port number (or any other port if you know what you are doing).', ['port' => $requiredPort]) !!}
                <br><br>
                <strong>{{ __('Example:') }}</strong> https://app.coolify.io:{{ $requiredPort }},https://www.app.coolify.io:{{ $requiredPort }}
            </x-callout>
        @endif

        <x-forms.input canGate="update" :canResource="$application" placeholder="https://app.coolify.io" :label="__('Domains')"
            id="fqdn"
