    ...
    <script data-navigate-once>
        Dropzone.options.myDropzone = {
            ...
            init: function () {
                ...
                button.innerText = @js(__('Select or drop a backup file here.'));
    ...
    @else
        ...
        @if (str($resourceStatus)->startsWith('running'))
            ...
            @if ($resourceDbType === 'standalone-postgresql')
                @if ($dumpAll)
                    <x-forms.textarea rows="6" readonly :label="__('Custom Import Command')"
                        wire:model='restoreCommandText'></x-forms.textarea>
                @else
                    <x-forms.input :label="__('Custom Import Command')" wire:model='postgresqlRestoreCommand'></x-forms.input>
                ...
                @endif
                <div class="w-64 pt-2">
                    <x-forms.checkbox :label="__('Backup includes all databases')" wire:model.live='dumpAll'></x-forms.checkbox>
                </div>
            @elseif ($resourceDbType === 'standalone-mysql')
                @if ($dumpAll)
                    <x-forms.textarea rows="14" readonly :label="__('Custom Import Command')"
                        wire:model='restoreCommandText'></x-forms.textarea>
                @else
                    <x-forms.input :label="__('Custom Import Command')" wire:model='mysqlRestoreCommand'></x-forms.input>
                @endif
                <div class="w-64 pt-2">
                    <x-forms.checkbox :label="__('Backup includes all databases')" wire:model.live='dumpAll'></x-forms.checkbox>
                </div>
            @elseif ($resourceDbType === 'standalone-mariadb')
                @if ($dumpAll)
                    <x-forms.textarea rows="14" readonly :label="__('Custom Import Command')"
                        wire:model='restoreCommandText'></x-forms.textarea>
                @else
                    <x-forms.input :label="__('Custom Import Command')" wire:model='mariadbRestoreCommand'></x-forms.input>
                @endif
                <div class="w-64 pt-2">
                    <x-forms.checkbox :label="__('Backup includes all databases')" wire:model.live='dumpAll'></x-forms.checkbox>
                </div>
            ...
            <h3 class="pt-6">{{ __('Choose Restore Method') }}</h3>
            <div class="flex gap-4 pt-2">
                <div @click="restoreType = 'file'"
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <h4 class="text-lg font-bold">{{ __('Restore from File') }}</h4>
                        <p class="text-sm text-neutral-600 dark:text-neutral-400">{{ __('Upload a backup file or specify a file path on the server') }}</p>
                    </div>
                </div>
                ...
                @if (count($availableS3Storages) > 0)
                    <div @click="restoreType = 's3'"
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <h4 class="text-lg font-bold">{{ __('Restore from S3') }}</h4>
                            <p class="text-sm text-neutral-600 dark:text-neutral-400">{{ __('Download and restore a backup from S3 storage') }}</p>
                        </div>
                    </div>
                @endif
            </div>
            ...
            @can('update', $this->resource)
                <div x-show="restoreType === 'file'" class="pt-6">
                    <h3>{{ __('Backup File') }}</h3>
                    <form class="flex gap-2 items-end pt-2">
                        <x-forms.input :label="__('Location of the backup file on the server')" :placeholder="__('e.g. /home/user/backup.sql.gz')"
                            wire:model='customLocation' x-model="$wire.customLocation"></x-forms.input>
                        <x-forms.button class="w-full" wire:click='checkFile' x-bind:disabled="!$wire.customLocation">{{ __('Check File') }}</x-forms.button>
                    </form>
                    ...
                    <div x-show="filename && !error" class="pt-6">
                        ...
                        <div class="pt-2">{{ __('Location:') }} <span x-text="filename ?? 'N/A'"></span><span x-show="filesize" x-text="' / ' + filesize"></span></div>
                        <div class="pt-2">
                            <x-modal-confirmation :title="__('Restore Database from File?')" :buttonTitle="__('Restore from File')"
                                submitAction="runImport" isErrorButton>
                                <x-slot:button-title>
                                    {{ __('Restore Database from File') }}
                                </x-slot:button-title>
                                {{ __('This will perform the following actions:') }}
                                <ul class="list-disc list-inside pt-2">
                                    <li>{{ __('Copy backup file to database container') }}</li>
                                    <li>{{ __('Execute restore command') }}</li>
                                </ul>
                                <div class="pt-2 font-bold text-error">{{ __('WARNING: This will REPLACE all existing data!') }}</div>
            ...
            @if (count($availableS3Storages) > 0)
                @can('update', $this->resource)
                    <div x-show="restoreType === 's3'" class="pt-6">
                        <h3>{{ __('Restore from S3') }}</h3>
                        <div class="flex flex-col gap-2 pt-2">
                            <x-forms.select :label="__('S3 Storage')" wire:model.live="s3StorageId">
                                <option value="">{{ __('Select S3 Storage') }}</option>
                            ...
                            <x-forms.input :label="__('S3 File Path (within bucket)')"
                                :helper="__('Path to the backup file in your S3 bucket, e.g., /backups/database-2025-01-15.gz')"
                            ...
                            @if ($s3FileSize)
                                <div class="pt-6">
                                    <h3>{{ __('File Information') }}</h3>
                                    <div class="pt-2">{{ __('Location:') }} {{ $s3Path }} {{ formatBytes($s3FileSize ?? 0) }}</div>
                                    <div class="pt-2">
                                        <x-modal-confirmation :title="__('Restore Database from S3?')" :buttonTitle="__('Restore from S3')"
                                            submitAction="restoreFromS3" isErrorButton>
                                            <x-slot:button-title>
                                                {{ __('Restore Database from S3') }}
                                            </x-slot:button-title>
                                            {{ __('This will perform the following actions:') }}
                                            <ul class="list-disc list-inside pt-2">
                                                <li>{{ __('Download backup from S3 storage') }}</li>
                                                <li>{{ __('Copy file into database container') }}</li>
                                                <li>{{ __('Execute restore command') }}</li>
                                            </ul>
                                            <div class="pt-2 font-bold text-error">{{ __('WARNING: This will REPLACE all existing data!') }}</div>
                                        </x-modal-confirmation>
            ...
            <x-slide-over @databaserestore.window="slideOverOpen = true" closeWithX fullScreen>
                <x-slot:title>{{ __('Database Restore Output') }}</x-slot:title>
                <x-slot:content>
                    <div wire:ignore>
                        <livewire:activity-monitor wire:key="database-restore-{{ $resourceUuid }}" :header="__('Logs')" fullHeight />
                    </div>
                </x-slot:content>
            </x-slide-over>
        @else
