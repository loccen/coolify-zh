...
<div>
    <div :class="collapsed && 'lg:hidden'">
        <x-forms.select wire:model.live="selectedTeamId">
            <option value="default" disabled selected>{{ __('navigation.team_switch') }}</option>
    ...
    <div class="hidden"
        ...
        <button @click="openTeamMenu($event)" type="button"
            title="{{ __('navigation.team_current', ['name' => $currentTeam->name]) }}"
        ...
        <div x-show="teamOpen"
            ...
            <div class="px-3 py-1.5 text-xs font-semibold text-neutral-500 dark:text-neutral-400 border-b border-neutral-200 dark:border-coolgray-200">{{ __('navigation.team_switch') }}</div>
            @foreach (auth()->user()->teams as $team)
                <button type="button"
