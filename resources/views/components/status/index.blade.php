...
<div class="flex flex-wrap items-center gap-1">
    ...
    @if (isset($resource->restart_count) && $resource->restart_count > 0 && !str($resource->status)->startsWith('exited'))
        <div class="flex items-center">
            <span class="text-xs dark:text-warning" title="{{ __('Container restarted :count times. Last restart: :lastRestart', ['count' => $resource->restart_count, 'lastRestart' => $resource->last_restart_at?->diffForHumans()]) }}">
                ({{ __(':count restarts', ['count' => $resource->restart_count]) }})
            </span>
        </div>
    @endif
    @if (!str($resource->status)->contains('exited') && $showRefreshButton)
        <button wire:loading.remove.delay.shortest wire:target="manualCheckStatus" title="{{ __('Refresh Status') }}" wire:click='manualCheckStatus'
        ...
        <button wire:loading.delay.shortest wire:target="manualCheckStatus" title="{{ __('Refreshing Status') }}" wire:click='manualCheckStatus'
            class="dark:hover:fill-white fill-black dark:fill-warning">
