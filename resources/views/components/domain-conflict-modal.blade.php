            ...
            <div x-show="modalOpen"
                ...
                <div x-show="modalOpen" x-trap.inert.noscroll="modalOpen" x-transition:enter="ease-out duration-100"
                    ...
                    <div class="flex justify-between items-center pb-3">
                        <h2 class="pr-8 font-bold">{{ __('Domain Already In Use') }}</h2>
                    ...
                    <div class="relative w-auto">
                        <x-callout type="danger" :title="__('Domain Conflict Detected')" class="mb-4">
                            {{ __('The following domain(s) are already in use by other resources. Using the same domain for multiple resources can cause routing conflicts and unpredictable behavior.') }}
                        </x-callout>
                            ...
                            <ul class="space-y-2">
                                @foreach ($conflicts as $conflict)
                                    <li class="flex items-start text-red-500">
                                        <div>
                                            <strong>{{ $conflict['domain'] }}</strong> {{ __('is used by') }}
                                            ...
                                            @endif
                                            （{{ __(str($conflict['resource_type'])->headline()->value()) }}）
                                        </div>
                                    </li>
                                @endforeach
                            </ul>
                        ...
                        <div class="flex flex-wrap gap-2 justify-between mt-4">
                            ...
                            <x-forms.button wire:click="{{ $confirmAction }}" @click="modalOpen = false" class="w-auto"
                                isError>
                                {{ __('I understand, proceed anyway') }}
                            </x-forms.button>
                        </div>
                    </div>
                </div>
