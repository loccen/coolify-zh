<dialog id="{{ $modalId }}" class="modal">
                    ...
                    @else
                        <div class="flex gap-4 mt-4">
                            <x-forms.button class="w-24 bg-coolgray-200 hover:bg-coolgray-100" type="button"
                                onclick="{{ $modalId }}.close()">{{ __('button.cancel') }}
                            </x-forms.button>
                            <div class="flex-1"></div>
                            <x-forms.button class="w-24" isError type="button"
                                wire:click.prevent='{{ $action }}' onclick="{{ $modalId }}.close()">{{ __('button.continue') }}
                            </x-forms.button>
                        </div>
                    @endif
                </div>
    ...
    <form method="dialog" class="modal-backdrop">
        <button aria-label="{{ __('button.close') }}">{{ __('button.close') }}</button>
    </form>
</dialog>
