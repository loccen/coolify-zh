@extends('layouts.base')
<div class="flex flex-col items-center justify-center h-full">
    <div>
        <p class="font-mono font-semibold text-7xl dark:text-warning">401</p>
        <h1 class="mt-4 font-bold tracking-tight dark:text-white">{{ __('You shall not pass!') }}</h1>
        <p class="text-base leading-7 dark:text-neutral-400 text-black">{{ __("You don't have permission to access this page.") }}
        </p>
        <div class="flex items-center mt-10 gap-x-2">
            <a href="{{ url()->previous() }}">
                <x-forms.button>{{ __('Go back') }}</x-forms.button>
            </a>
            <a href="{{ route('dashboard') }}" {{ wireNavigate() }}>
                <x-forms.button>{{ __('Dashboard') }}</x-forms.button>
            </a>
            <a target="_blank" class="text-xs" href="{{ config('constants.urls.contact') }}">{{ __('Contact support') }}
                <x-external-link />
            </a>
        </div>
    </div>
</div>
