...
- `coolify-i18n-maintenance` — Use whenever the task is to continue, complete, or correct Coolify internationalization work: untranslated pages, wrong `zh_CN` copy, missing translation wiring, seed/demo text still in English, notification or mail channel text cleanup, locale regression coverage, or terminology fixes such as keeping `Docker` as `Docker` instead of mistranslating it.
